local function createManager(config)
	config = config or {}
	-- Configuration
	local MANAGER_ID = config.managerId or "manager"
	local MANAGER_VERSION = "2025.12.19.0804"
	...
	local reloadManager = config.reloadManager
	...
	local function getManagerDef()
		return getPackageDef(MANAGER_ID)
	end
	...
	buildMenu = function()
		...
		-- System section
		local managerDef = getManagerDef()
		if managerDef and managerDef.version and managerDef.version ~= MANAGER_VERSION then
			table.insert(menu, {
				title = "Update Power Spoons (" .. tostring(managerDef.version) .. ")",
				fn = function()
					downloadPackage(managerDef, function(success, err)
						if not success then
							reportError("Failed to update Power Spoons", err, true)
							return
						end
						if type(reloadManager) == "function" then
							reloadManager()
						else
							notify("Power Spoons", "Update installed. Reload Hammerspoon to apply.")
						end
					end)
				end,
			})
